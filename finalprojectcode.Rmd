...
```{r}
trial_data2 <- read_csv("C:/Users/huifa/Desktop/BCB Research/trial_data2.csv")
...
d + stat_summary(fun.data = "mean_cl_boot", colour = "red", size = .55) + facet_wrap(~key, ncol = 1) +
  ...
  theme_bw()
ggsave("trials.pdf", width = 4, height = 10, units = "in")
```
...
emo <- read.csv('C:/Users/huifa/Desktop/BCB Research/emotion_data2.csv', header=TRUE)
emo2 <- emo %>%  
  gather(
    `emotions_joy`, 
    `emotions_sadness`, 
...
ggplot(emo2) +
  ...
  theme_bw()
ggsave("emotions.pdf", width = 20, height = 10, units = "in")
```

Survey Data Charts
```{r}
survey <- read.csv('C:/Users/huifa/Desktop/BCB Research/survey_data.csv', header=TRUE)
...
ggsave("survey.pdf", width = 16, height = 10, units = "in")
...
ggsave("barchart.pdf", width = 14, height = 20, units = "in")
```
...
ggplot(emo2) +
  ...
  theme_bw()
ggsave("emotionsneg.pdf", width = 20, height = 9, units = "in")
```
...
ggplot(emo2) +
  ...
  theme_bw()
ggsave("emotionspos.pdf", width = 20, height = 10, units = "in")
```
...
ggplot(emo2) +
  ...
  theme_bw()
ggsave("emotionsvalandeng.pdf", width = 20, height = 9, units = "in")
```
Error Mean Values
```{r}
trial_data2 <- read_csv("C:/Users/huifa/Desktop/BCB Research/trial_data2.csv")
